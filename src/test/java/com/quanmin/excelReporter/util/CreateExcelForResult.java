package com.quanmin.excelReporter.util;
import java.io.File;
...
import com.quanmin.appium.PropertiesUtil;

import qmtv.email.DateUtil;
import qmtv.email.EmailConfig;
...
public class CreateExcelForResult{
	...
	static String jobname = "qmtv_android";
	static int runCaseCount=0;
	static int passedCount=0;
	static int failedCount=0;
	static int skipedCount=0;
	...
	public static void createExcelReport(List<XmlSuite> xml, List<ISuite> suites,String dir) {
        ...
        jobname = suites.get(0).getName()+"_"+AppiumUtil.version;
        cellD2F2.setCellValue(jobname);
        //运行开始时间
        Date startTime=null;
        //运行结束时间
        Date endTime = null;
    		...
    		for (int i = 0; i < testNum; i++) {
    			...
    			for (int j = 0; j < 6; j++) {
    				...
    				//用例总数	
        	    			...
        	    			runCaseCount = runCaseCount + totalCases[i];
        	    			cell.setCellStyle(setContentCellBorder(workbook));
    				}		
    				...
    				if(j==3){
 					...
 					passedCount = passedCount + successCases[i];
 					cell.setCellStyle(setContentCellBorder(workbook));
    				}
    				if(j==4){
					//失败条数
 					cell.setCellValue(failCases[i]);
 					failedCount = failedCount + failCases[i];
 					cell.setCellStyle(setContentCellBorder(workbook));
    				}
	   		...
	   		for (ISuiteResult r : tests.values()) {//多少个模块第一层循环
    			...
    			if(failTestNum!=0){//处理失败的
				 ...
				 for (ITestNGMethod im : overview.getFailedTests().getAllMethods()) {	//模块下有多少个用例循环
 					...
 					String functionPack = caseName.substring(0, caseName.indexOf("_")).toLowerCase();
	...
	public static void sendEmail() {
		System.out.println("************************");
		System.out.println("测试完成的job是:"+jobname);
		System.out.println("runCaseCount="+runCaseCount);
		System.out.println("passedCount="+passedCount);
		System.out.println("failedCount="+failedCount);
		System.out.println("************************");
		ReportTemplate.caseCount = runCaseCount;
		ReportTemplate.successCount = passedCount;
		ReportTemplate.failureCount = failedCount;
		
		// 获取配置文件里关于邮箱的相关信息
		String smtpHost=PropertiesUtil.getProperty("smtpHost");
		String sender=PropertiesUtil.getProperty("sender");
		String sendPassword=PropertiesUtil.getProperty("sendPassword");
		String receivers=PropertiesUtil.getProperty("receivers");
		String chaosong=PropertiesUtil.getProperty("chaosong");
		String subject=PropertiesUtil.getProperty("subject")+"_"+AppiumUtil.version;
		
		EmailConfig ec = new EmailConfig.Builder()
				.setSmtpHost(smtpHost)
				.setSender(sender)
				.setSendPassword(sendPassword)
				.setReceivers(receivers)
				.setChaosong(chaosong)
				.setSubject(subject)
				.build();
		

		if(!EmailConfig.receivers.equals("")){
			System.out.println("==邮件收件人是=="+EmailConfig.receivers);
			System.out.println("==邮件抄送人是=="+EmailConfig.chaosong);
			System.out.println("==邮件主题是=="+EmailConfig.subject);
			System.out.println("==开始发送邮件>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
			System.out.println("报告的绝对路径是:"+fileForExcel.getAbsolutePath());
			new ReportTemplate(EmailConfig.receivers, EmailConfig.chaosong, EmailConfig.subject).sendReport(fileForExcel.getAbsolutePath());
		}
	}
